...
The Mobile Inspector now sends either an uploaded QR image or a captured live-camera frame to `POST /api/opencv/qr/decode`. The FastAPI backend decodes it with OpenCV and verifies the decoded permit against `/api/vendors`; it no longer returns a hard-coded vendor.

OpenCV footfall frames can be submitted to `POST /api/opencv/footfall/count`. Zone Performance reads backend zone records and the latest fused observation, while What-If results are generated by the pipeline response rather than frontend constants. Officers can run the protected Enforcement Intelligence workflow, and vendors can submit real income values from My Vendor Profile to execute livelihood tracking. Set `NOTIFICATION_WEBHOOK_URL` to enable notification delivery; without it, the pipeline reports `not_configured` instead of claiming a message was sent.

For local use, install `backend/requirements.txt`, start FastAPI on port 8000, then start the Vite frontend. Browser camera access requires `localhost` or HTTPS.
