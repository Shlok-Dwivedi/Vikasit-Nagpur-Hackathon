...
import { Smartphone, QrCode, ShieldAlert, CheckCircle2, MapPin, Camera, AlertTriangle, VideoOff, RefreshCw, Cpu, TrendingUp } from 'lucide-react';
...
export default function MobileInspector({ backendUrl }) {
  ...
  const [pipeline4Result, setPipeline4Result] = useState(null);
  ...
  // Trigger Agentic Pipeline 4: Enforcement-to-Permanent-Zoning
  const handleLogViolation = async (e) => {
    e.preventDefault();
    setViolationLogged(true);

    try {
      const res = await fetch(`${apiBackendUrl}/api/pipelines/enforcement-to-zoning`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ location: 'Nagpur Market Sq', violations_count: 4 })
      });
      const data = await res.json();
      setPipeline4Result(data);
    } catch (err) {
      console.warn('Pipeline 4 execution note:', err);
    }

    setTimeout(() => setViolationLogged(false), 5000);
  };
  ...
  return (
    <div className="inspector-container">
      <div className="mobile-frame">
        ...
        <form className="violation-form" onSubmit={handleLogViolation}>
          ...
          {violationLogged && (
            <div className="status-msg success">
              ...
              <span>Violation geotagged! Triggering Pipeline #4 Enforcement Intel...</span>
            </div>
          )}
        ...
        {/* Pipeline #4 Result Banner */}
        {pipeline4Result && (
          <div className="scan-result-card" style={{ borderColor: 'rgba(59, 130, 246, 0.4)', background: 'var(--input-bg)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#60a5fa', fontSize: '0.8rem', fontWeight: '700', marginBottom: '4px' }}>
              <Cpu size={14} />
              <span>Agentic Pipeline #4: Enforcement-to-Zoning</span>
            </div>
            <p style={{ fontSize: '0.78rem', color: 'var(--text-main)', lineHeight: '1.4' }}>
              {pipeline4Result.recommendation}
            </p>
          </div>
        )}
