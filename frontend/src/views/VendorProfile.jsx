...
import { User, QrCode, ShieldCheck, MapPin, Phone, Calendar, IndianRupee, Download, CheckCircle2, Clock, Building2, Store, Cpu } from 'lucide-react';
import './VendorProfile.css';

export default function VendorProfile({ currentUser, backendUrl }) {
  const [vendorData, setVendorData] = useState(null);
  const [pipeline5Result, setPipeline5Result] = useState(null);
    ...
    fetch(`${apiBackendUrl}/api/vendors`)
      ...
      .then((data) => {
        ...
        if (data.vendors && data.vendors.length > 0) {
          match = data.vendors.find(v => v.name?.toLowerCase().includes(currentUser?.name?.toLowerCase())) || data.vendors[0];
          setVendorData(match);
        } else {
          match = {
            id: 'VV-2024-001',
        ...
        // Trigger Agentic Pipeline 5: Vendor Certification & Livelihood Tracking
        fetch(`${apiBackendUrl}/api/pipelines/livelihood-tracking`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ vendor_id: match.id, vendor_name: match.name })
        })
          .then(r => r.json())
          .then(p5Data => setPipeline5Result(p5Data))
          .catch(e => console.warn('Pipeline 5 fetch note:', e));
      })
    ...
    <div className="profile-container">
      <div className="section-header">
        <div>
          ...
          <span className="sub-header-tag">Executing Agentic Pipeline #5: Livelihood & Certification</span>
        </div>
        ...
        <div className="ai-card">
          ...
          <div className="profile-fields-grid">
            ...
            <div className="profile-field">
              ...
              <p style={{ color: '#34d399' }}>{pipeline5Result?.pm_svanidhi_tier || vendorData?.svanidhiTier || 'Tier 1 Approved (₹10,000)'}</p>
            </div>
          </div>

          {/* Pipeline #5 Agentic Output Banner */}
          {pipeline5Result && (
            <div className="recommendation-box" style={{ borderColor: 'rgba(16, 185, 129, 0.4)', marginTop: '16px' }}>
              <div className="recommendation-header" style={{ color: '#10b981' }}>
                <Cpu size={16} />
                <span>Pipeline #5: Livelihood Impact Analysis</span>
              </div>
              <p style={{ fontSize: '0.85rem' }}>{pipeline5Result.recommendation}</p>
            </div>
          )}
        </div>
